"""
Name: Chris Jackson
Time To Completion: 9 hours (ish)
Comments:

Sources:
    My project 4 solution

    I had no idea how to start with DESC. I have never used itemgetter before this class and so when I saw it in the
        previous project solutions I researched it and thought I had a pretty good understanding of it but I wasn't sure how to
        use it to sort when given multiple keys (i.e. student.grade DESC, teacher.name). So I did some research and
        found this which was very helpful in understanding how to use itemgetter to sort with multiple keys:
            https://stackoverflow.com/questions/1143671/python-sorting-list-of-dictionaries-by-multiple-keys

            https://docs.python.org/3/howto/sorting.html - Used to learn more about sorted (specifically how to use a cusom comparison function)

            https://www.w3schools.com/python/ref_func_next.asp - Used to learn about iterating through generator objects

    I didn't realize at the time but the above sources for DESC also really helped with collations (custom comparison functions)
...
class Connection(object):
    ...
    def execute(self, statement):
        ...
        def create_table(tokens):
            ...
            _TABLE_DATA[table_name] = column_name_type_pairs
            ...
            while True:
                ...
                if comma_or_close == "DEFAULT":
                    next_token = tokens.pop(0)
                    default_dict[column_name] = next_token
                    comma_or_close = tokens.pop(0)
            ...
            _DEFAULT_DICT[table_name] = default_dict

            if self.open_transaction:
                self.transaction_database.create_new_table(table_name, column_name_type_pairs)
                self.transaction_database.tables[table_name].has_default = True
            else:
                self.database.create_new_table(table_name, column_name_type_pairs)
                self.database.tables[table_name].has_default = True

        def select_view(query, view_name):
            new_tokens = tokenize(query)
            rows = self.execute(query)

            pop_and_check(new_tokens, "SELECT")
            if new_tokens[0] == "DISTINCT":
                new_tokens.pop(0)
            ...
            while True:
                if not new_tokens:
                    break
                if new_tokens[0] == "FROM":
                    break
                if new_tokens[0] != "," and new_tokens[0] != "FROM":
                    output_columns.append(new_tokens.pop(0))
                else:
                    new_tokens.pop(0)
            
            final_output_columns = []
            while len(output_columns) > 0:
                final_output_columns.append(get_qualified_column_name(output_columns))

            pop_and_check(new_tokens, "FROM")
            left_table = new_tokens.pop(0)
            right_table = None
            if len(new_tokens) > 0:
                if new_tokens[0] == "LEFT":
                    pop_and_check(new_tokens, "LEFT")
                    new_tokens.pop(0)
                    pop_and_check(new_tokens, "JOIN")
                    right_table = new_tokens.pop(0)

            left_column_type_pairs = _TABLE_DATA[left_table]
            right_column_type_pairs = None
            if right_table:
                right_column_type_pairs = _TABLE_DATA[right_table]

            view_column_type_pairs = []
            if final_output_columns == [QualifiedColumnName("*", None)]:
                if right_table:
                    view_column_type_pairs = view_column_type_pairs + left_column_type_pairs + right_column_type_pairs
                else:
                    view_column_type_pairs = view_column_type_pairs + left_column_type_pairs
            else:
                for x in final_output_columns:
                    if x.col_name == "*":
                        if x.table_name == left_table:
                            view_column_type_pairs = view_column_type_pairs + left_column_type_pairs
                        elif x.table_name == right_table:
                            view_column_type_pairs = view_column_type_pairs + right_column_type_pairs
                        else:
                            raise Exception("Invalid table name")
                    else:
                        for item in left_column_type_pairs:
                            if item[0] == x:
                                view_column_type_pairs.append(item)
                                break
                        for item in right_column_type_pairs:
                            if item[0] == x:
                                view_column_type_pairs.append(item)
                                break
            ...
            for row in rows:
                ...
                for x in range(len(row)):
                    test[view_column_type_pairs[x][0]] = row[x]
                abc.append(test)

            return view_column_type_pairs, abc, left_table, right_table, rows

        def select_and_update_view(query, view_name):
            view_column_type_pairs, abc, left_table, right_table, rows = select_view(query, view_name)
            
            if view_name in self.database.tables.keys():
                self.database.tables[view_name].rows = copy.deepcopy(abc)
            else:
                self.database.create_new_table(view_name, view_column_type_pairs, True)
                self.database.tables[view_name].rows = abc
        ...
        def create_view(tokens):
            ...
            view_column_type_pairs, abc, left_table, right_table, rows = select_and_update_view(query, view_name)

            if left_table in _VIEWS_DICT.keys():
                _VIEWS_DICT[left_table].append((view_name, query, view_column_type_pairs))
            else:
                _VIEWS_DICT[left_table] = [(view_name, query, view_column_type_pairs)]

            if right_table:
                if right_table in _VIEWS_DICT.keys():
                    _VIEWS_DICT[right_table].append((view_name, query, view_column_type_pairs))
                else:
                    _VIEWS_DICT[right_table] = [(view_name, query, view_column_type_pairs)]
        ...
        def insert(tokens):
            ...
            # USE DEFAULT VALUES
            if tokens[0] == "DEFAULT":
                assert table_name in _DEFAULT_DICT.keys()

                default_dict = _DEFAULT_DICT[table_name]
                qual_col_names = []
                row_contents = []
                for key, value in default_dict.items():
                    qual_col_names.append(QualifiedColumnName(key, table_name))
            ...
            if len(tokens) == 0:
                ...
                while tokens:
                    ...
                    temp_qual_col_names = copy.deepcopy(qual_col_names)
                    
                    if temp_qual_col_names:
                        assert len(row_contents) == len(temp_qual_col_names)
                        
                    if self.open_transaction:
                        default = self.transaction_database.tables[table_name].has_default
                        
                        if len(self.transaction_database.tables[table_name].column_names) > len(row_contents) and default:
                            for x in self.transaction_database.tables[table_name].column_names:
                                if x not in temp_qual_col_names:
                                    temp_qual_col_names.append(x)
                                    row_contents.append(_DEFAULT_DICT[table_name][x.col_name])
                    else:
                        default = self.database.tables[table_name].has_default
                        
                        if len(self.database.tables[table_name].column_names) > len(row_contents) and default:
                            for x in self.database.tables[table_name].column_names:
                                if x not in temp_qual_col_names:
                                    temp_qual_col_names.append(x)
                                    row_contents.append(_DEFAULT_DICT[table_name][x.col_name])
                    ...
                    if self.open_transaction:
                        self.transaction_database.insert_into(table_name,
                                                row_contents,
                                                qual_col_names=temp_qual_col_names)
                    else:
                        self.database.insert_into(table_name,
                                                row_contents,
                                                qual_col_names=temp_qual_col_names)
            ...
            if table_name in _VIEWS_DICT.keys():
                for view in _VIEWS_DICT[table_name]:
                    self.database.tables[view[0]].rows = copy.deepcopy(self.database.tables[table_name].rows)
                    select_and_update_view(view[1], view[0])

        def get_qualified_column_name(tokens, table_name=None):
            """
        ...
        def select(tokens):
            ...
            if tokens[0] == "max" or tokens[0] == "min":
                while True:
                    if tokens[0] == "max":
                        min_or_max = True
                        pop_and_check(tokens, "max")
                        pop_and_check(tokens, "(")
                        min_or_max_col = tokens.pop(0)
                        pop_and_check(tokens, ")")
                        min_or_max_data.append(("max", min_or_max_col))
                    elif tokens[0] == "min":
                        min_or_max = True
                        pop_and_check(tokens, "min")
                        pop_and_check(tokens, "(")
                        min_or_max_col = tokens.pop(0)
                        pop_and_check(tokens, ")")
                        min_or_max_data.append(("min", min_or_max_col))
                    if tokens[0] == "FROM" and tokens[0] == "FROM":
                        break
                    else:
                        pop_and_check(tokens, ",")
            ...
            for item in min_or_max_data:
                output_columns.append( QualifiedColumnName(item[1], table_name) )
            ...
            while True:
                ...
                if tokens:
                    if tokens[0] == "COLLATE":
                        ...
                        collate_columns.append((qual_col_name, collation_name))
                if not tokens:
                    desc_columns.append("ASC")
                    break
                if tokens[0] == "DESC":
                    pop_and_check(tokens, "DESC")
                    desc_columns.append("DESC")
                else:
                    desc_columns.append("ASC")
                if not tokens:
                    break
            ...
            if self.open_transaction:
                if self.autocommit:
                    _LOCKS.pop(hash(self))
                returnData = list(self.transaction_database.select(
                    output_columns,
                    order_by_columns,
                    desc_columns,
                ...
                if self.autocommit:
                    _LOCKS.pop(hash(self))
                returnData = list(self.database.select(
                    output_columns,
                    order_by_columns,
                    desc_columns,
                    from_join_clause=from_join_clause,
                ...
                if min_or_max:
                    data = []
                    for indx in range(len(min_or_max_data)):
                        if min_or_max_data[indx][0] == "max":
                            data.append( get_max(returnData, indx) )
                        else:
                            data.append( get_min(returnData, indx) )
        ...
        def get_min(data, indx):
            min_item = data[0][indx]
            for item in data:
                if item[indx] < min_item:
                    min_item = item[indx]

            return min_item

        def get_max(data, indx):

            max_item = data[0][indx]
            for item in data:
                if item[indx] > max_item:
                    max_item = item[indx]
        ...
        if tokens[0] == "CREATE":
            if tokens[1] == "TABLE":
                ...
                raise AssertionError("Unexpected first word in statements: " + tokens[0] + " " + tokens[1])
    ...
    def executemany(self, statement, data):
        ...
        for item in data:
            ...
            for value in item:
                statement = statement.replace("?", str(value), 1)
            self.execute(statement)

    def create_collation(self, name, sorting_function):
        _COLLATIONS[name] = sorting_function
...
class Database:
    ...
    def create_new_table(self, table_name, column_name_type_pairs, isView=False):
        assert table_name not in self.tables
        self.tables[table_name] = Table(table_name, column_name_type_pairs, isView)
        return []
    ...
    def select(self, output_columns, order_by_columns, desc_columns, from_join_clause,
               collate_columns=None, where_clause=None, is_distinct=False):
        ...
        return table.select_rows(output_columns, order_by_columns, desc_columns,
                                 where_clause=where_clause,
                                 is_distinct=is_distinct, collate_columns=collate_columns)
####################
## Database class
####################
...
class Table:
    ...
    def __init__(self, name, column_name_type_pairs, isView=False):
        self.name = name
        if column_name_type_pairs:
    ...
    def _row_match_where(self, row, where_clause):
        ...
        if self.isView:
            for value in _VIEWS_DICT.values():
                for item in value:
                    found = False
                    if item[0] == self.name:
                        view_column_type_pairs = item[2]
                        found = True
                        break
                if found:
                    break
            for item in view_column_type_pairs:
                if item[0].col_name == where_clause.col_name.col_name:
                    where_clause.col_name.table_name = item[0].table_name
    ...
    def select_rows(self, output_columns, order_by_columns, desc_columns,
                    where_clause=None, is_distinct=False, collate_columns=None):
            ...
            if self.isView:
                ...
                for col in columns:
                    if col.table_name is None:
                        for item in view_column_type_pairs:
                            if item[0].col_name == col.col_name:
                                col.table_name = item[0].table_name
            else:
                for col in columns:
                    if col.table_name is None:
                        col.table_name = self.name

        def sort_rows(rows, order_by_columns, desc_columns, collate_columns):

            def sort_by_keys(items, columns):
                def cmp(left, right):
                    if left > right:
                        return 1
                    elif left < right:
                ...
                def comparer(left, right):
                    comparisons = []
                    collate_column_names = []
                    for x, y in collate_columns:
                        collate_column_names.append(x)
                    ctr = 0
                    for x, y in comparers:
                        if cols[ctr] in collate_column_names:
                            for column, name in collate_columns:
                                if column == cols[ctr]:
                                    collate_sort_function = _COLLATIONS[name]
                            comparisons.append(collate_sort_function(x(left), x(right)) * y)
                        else:
                            comparisons.append(cmp(x(left), x(right)) * y)
                        ctr += 1

                    def createComparisonGenerator(comparisons):
                        for result in comparisons:
                            if result:
                                yield result

                    comparisonGenerator = createComparisonGenerator(comparisons)

                    return next(comparisonGenerator, 0)
                
                comparers = []
                cols = []
                for col in columns:                    
                    if col.col_name.startswith('-'):
                        col.col_name = col.col_name[1:]
                        comparers.append((itemgetter(col), -1))
                        cols.append(col)
                    else: 
                        comparers.append((itemgetter(col), 1))
                        cols.append(col)
            ...
            if len(desc_columns) > 0:
                for x in range(len(order_by_columns)):
                    if desc_columns[x] == "DESC":
                        updated_order_by_columns.append(QualifiedColumnName("-"+order_by_columns[x].col_name, order_by_columns[x].table_name))
                    else:
                        updated_order_by_columns.append(QualifiedColumnName(order_by_columns[x].col_name, order_by_columns[x].table_name))
            else:
                for x in range(len(order_by_columns)):
            ...
            return sort_by_keys(rows, updated_order_by_columns)
        ...
        sorted_rows = sort_rows(filtered_rows, order_by_columns, desc_columns, collate_columns)
